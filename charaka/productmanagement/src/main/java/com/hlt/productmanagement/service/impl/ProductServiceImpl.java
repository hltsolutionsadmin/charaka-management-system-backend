...
import com.hlt.productmanagement.client.UserMgmtClient;
import com.hlt.productmanagement.dto.B2BUnitDTO;
import com.hlt.productmanagement.dto.BusinessWithProductsDTO;
import com.hlt.productmanagement.dto.ProductAttributeDTO;
...
import com.hlt.productmanagement.model.ProductAttributeModel;
import com.hlt.productmanagement.model.ProductModel;
import com.hlt.productmanagement.populator.ProductPopulator;
import com.hlt.productmanagement.repository.CategoryRepository;
import com.hlt.productmanagement.repository.ProductRepository;
import com.hlt.productmanagement.service.MediaService;
import com.hlt.productmanagement.service.ProductAvailabilityScheduler;
import com.hlt.productmanagement.service.ProductService;
import com.hlt.productmanagement.utils.ExcelHelper;
...
//    @CachePut(value = "products", key = "#result.id")
    ...
    private B2BUnitDTO validateBusinessApproval(Long businessId) {
        ...
        if (business == null || !business.isApproved()) {
            throw new HltCustomerException(ErrorCode.RESTAURANT_NOT_APPROVED);
        }
        return business;
    }
...
//    )
    ...
    @Override
    @Transactional
    public String deleteProductsByIds(List<Long> ids) {
        var products = productRepository.findAllById(ids);
        if (products.isEmpty()) {
            throw new HltCustomerException(ErrorCode.PRODUCT_NOT_FOUND);
        }
        productRepository.deleteAll(products);
        return "Deleted products successfully: " + ids;
    }
    ...
    public List<ProductDTO> bulkUploadProducts(MultipartFile file) throws IOException {
        ...
        // Parse Excel into a list of ProductDTOs
        List<ProductDTO> productDTOs = ExcelHelper.parseProductsExcel(file);

        List<ProductDTO> savedProducts = new ArrayList<>();
        for (ProductDTO dto : productDTOs) {
            try {
                savedProducts.add(createOrUpdateProduct(dto));
            } catch (HltCustomerException e) {
                log.warn("Skipping product '{}' due to error: {}", dto.getName(), e.getErrorCode().getMessage());
            }
        }
