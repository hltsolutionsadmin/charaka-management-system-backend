package com.juvarya.user.access.mgmt.azure.service.impl;
...
public class AwsBlobServiceImpl implements AwsBlobService {
    ...
    @Override
    public MediaModel uploadB2BMedia(Long b2bUnitId, MultipartFile file, TimeSlot timeSlot, Long createdBy) throws IOException {
        File modified = new File(file.getOriginalFilename());
        try (FileOutputStream os = new FileOutputStream(modified)) {
            os.write(file.getBytes());
        }
        ...
        media.setName("B2B Media: " + timeSlot);
        media.setCustomerId(null);
        media.setActive(true);

        media.setB2bUnitModel(mediaService.getB2BUnitReference(b2bUnitId));
