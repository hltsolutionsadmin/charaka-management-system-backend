...
import com.juvarya.commonservice.dto.*;
...
import com.juvarya.user.access.mgmt.dto.BasicUserDetails;
...
import com.juvarya.utils.AbstractConverter;
...
import static com.juvarya.commonservice.enums.ERole.ROLE_DELIVERY_PARTNER;
...
public class UserEndpoint extends JTBaseEndpoint {
    ...
    public ResponseEntity<StandardResponse<String>> updateBasicUserDetails(
        ...
        ProjectLoginFlags flags = userModel.getProjectLoginFlags();
        if (flags == null) {
            flags = new ProjectLoginFlags();
            userModel.setProjectLoginFlags(flags);
        }

        if (details.getSkillrat() != null) {
            flags.setSkillrat(details.getSkillrat());
        }
        if (details.getYardly() != null) {
            flags.setYardly(details.getYardly());
        }
        if (details.getEato() != null) {
            flags.setEato(details.getEato());
        }
        if (details.getSancharalakshmi() != null) {
            flags.setSancharalakshmi(details.getSancharalakshmi());
        }
        if (details.getDeliveryPartner() != null) {
            flags.setDeliveryPartner(details.getDeliveryPartner());
        }
    ...
    @GetMapping("/admin/unverified-students")
    public ResponseEntity<StandardResponse<Page<UserDTO>>> getUnverifiedStudents(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {

        Page<UserDTO> unverifiedStudents = userService.getUnverifiedStudents(page, size);
        return ResponseEntity.ok(
                new StandardResponse<>("Fetched unverified students", "SUCCESS", unverifiedStudents)
        );
    }
    ...
    @DeleteMapping("/skillrat")
    public ResponseEntity<StandardResponse<?>> deleteRolesAndResetSkillrat() {
        UserDetailsImpl loggedInUser = SecurityUtils.getCurrentUserDetails();
        userService.deleteRolesAndResetSkillratFlag(loggedInUser.getId());
        return ResponseEntity.ok(StandardResponse.message("User deleted "));
    }

    @PreAuthorize("hasRole('ROLE_USER_ADMIN')")
    @PutMapping("/approve-delivery-partner/{userId}")
    public ResponseEntity<StandardResponse<String>> approveDeliveryPartner(
            @PathVariable Long userId,
            @AuthenticationPrincipal UserDetailsImpl loggedInUser) {

        userService.approveDeliveryPartner(userId);
        return ResponseEntity.ok(StandardResponse.message("User approved as delivery partner successfully."));
    }

    @GetMapping("/deliver-unverified-list")
    public ResponseEntity<StandardResponse<List<UserDTO>>> getDeliveryUnflaggedUsersByRole(
           ) {
        ERole roleEnum=ROLE_DELIVERY_PARTNER;

        List<UserDTO> users = userService.getUsersByRoleAndDeliveryPartnerFalse(roleEnum);
        return ResponseEntity.ok(StandardResponse.list("Delivery UnflaggedUsersByRole fetched succesfully",users));
    }
}
